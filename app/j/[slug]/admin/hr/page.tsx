import { notFound } from "next/navigation";
import { getTenantConfigBySlugServer } from "@/lib/utils/tenant-server";
import { AdminNav } from "../admin-nav";
import { ShiftsList } from "./shifts-list";

export default async function AdminHRPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await params;
  const tenant = await getTenantConfigBySlugServer(slug);
  if (!tenant) notFound();
  return (
    <div className="min-h-screen p-6">
      <AdminNav slug={slug} tenantName={tenant.name} />
      <h2 className="text-xl font-semibold mb-4">HR / Attendance</h2>
      <p className="text-sm text-gray-500 mb-4">Shift history. Set geofence in Settings to restrict check-in/out to the venue.</p>
      <ShiftsList slug={slug} />
    </div>
  );
}
