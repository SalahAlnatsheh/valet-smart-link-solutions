...
export async function POST(
  ...
  if (geofence) {
    ...
    if (!within) {
      return NextResponse.json(
        { error: "Check-out only allowed within the venue area. Please enable location." },
        { status: 400 }
      );
    }
  }
  ...
  if (!openShift) {
    return NextResponse.json({ error: "No active shift to check out" }, { status: 400 });
  }

  const now = new Date().toISOString();
  const shiftDoc = openShift;
  await shiftDoc.ref.update({
    checkOutAt: now,
    checkOutLocation: { lat, lng, ...(typeof accuracy === "number" && { accuracy }) },
    ...(deviceId && { deviceId }),
  });

  return NextResponse.json({ success: true, shiftId: shiftDoc.id });
}
